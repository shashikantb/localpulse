...
const UserProfilePage: FC<UserProfilePageProps> = async ({ params }) => {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 bg-yellow-300 text-black p-2 z-[9999] text-xs w-full border-t-2 border-black font-mono">
        <h3 className="font-bold">Debug Info (Production):</h3>
        <pre>
          - Session User: {sessionUser ? `ID: ${sessionUser.id}, Name: ${sessionUser.name}` : 'NULL'}
        </pre>
        <pre>
          - Is Own Profile: {isOwnProfile ? 'true' : 'false'}
        </pre>
        <pre>
          - Family Status Result: {JSON.stringify(familyStatusResult)}
        </pre>
      </div>
      {/* --- END DEBUGGING BLOCK --- */}
