...
const formSchema = z.object({
  ...
  is_family_post: z.boolean().default(false),
});

type FormData = z.infer<typeof formSchema>;

interface PostFormProps {
  onSubmit: (content: string, hashtags: string[], is_family_post: boolean, mediaUrls?: string[], mediaType?: 'image' | 'video' | 'gallery', mentionedUserIds?: number[]) => Promise<void>;
  submitting: boolean;
}
  ...
  const handleSubmitForm: SubmitHandler<FormData> = async (data) => {
      ...
      if (selectedFiles.length > 0) {
        ...
        try {
          ...
          await onSubmit(data.content, hashtagsToSubmit, data.is_family_post, uploadedUrls, finalMediaType, mentionedUserIds);
        ...
        await onSubmit(data.content, hashtagsToSubmit, data.is_family_post, undefined, undefined, mentionedUserIds);
      }
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm bg-muted/50">
              <div className="space-y-0.5">
                <FormLabel className="flex items-center"><Users className="mr-2 h-4 w-4 text-primary"/>Family Only Post</FormLabel>
                <FormMessage>
                  <p className="text-xs text-muted-foreground">If checked, this post will only be visible to your approved family members.</p>
                </FormMessage>
              </div>
              <FormControl>
                <Switch
