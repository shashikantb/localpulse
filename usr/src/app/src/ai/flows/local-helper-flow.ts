...
const findNearbyBusinessesTool = defineTool(
  {
  ...
  async (input, context) => {
    const flowContext = context as LocalHelperInput;
    if (!flowContext.latitude || !flowContext.longitude) {
    ...
    const businesses = await getNearbyBusinessesDb({
      latitude: flowContext.latitude,
      longitude: flowContext.longitude,
...
export const localHelperFlow = defineFlow(
  {
